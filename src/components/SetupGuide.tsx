...
export const SetupGuide: React.FC<SetupGuideProps> = ({ theme = 'matrix' }) => {
  ...
  const [copiedCleanGit, setCopiedCleanGit] = useState(false);
  const gitCleanCommand = `rm -rf assets public src index.html metadata.json .env.example && git add . && git commit -m "Cleaned repo: Removed web UI files, kept Termux script only" && git push origin main`;

  const handleCopyCleanGit = () => {
    navigator.clipboard.writeText(gitCleanCommand);
    setCopiedCleanGit(true);
    setTimeout(() => setCopiedCleanGit(false), 2000);
  };
  ...
  return (
    ...
    <div className="space-y-6 font-mono">
      ...
      {/* GitHub Repo Cleanup Card */}
      <div className={`bg-slate-950 border-2 border-rose-500/50 rounded-2xl p-5 shadow-2xl space-y-3 relative overflow-hidden`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="flex items-center space-x-2">
            <Terminal className="w-5 h-5 text-rose-400 animate-pulse" />
            <h3 className="text-sm font-black text-rose-400 tracking-wider uppercase">
              🧹 GitHub Repository Cleanup & Push Command
            </h3>
          </div>
          <span className="text-[10px] px-2.5 py-1 rounded-full font-bold border border-rose-500/40 text-rose-300 bg-rose-950/80">
            Keep Termux Script Only
          </span>
        </div>

        <p className="text-xs text-slate-300">
          Run this in your local Git repository terminal to remove web app assets (<code className="text-rose-400">assets, public, src, index.html</code>) and push only the pure Termux Python & PHP scripts to <code className={style.accent}>https://github.com/itzgeniusboy/FeaturesticLeaks-Toolkit-.git</code>.
        </p>

        <div className="p-3.5 bg-black/95 rounded-xl border border-rose-950 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <code className="text-xs text-rose-300 font-extrabold overflow-x-auto whitespace-nowrap select-all pr-2 w-full">
            {gitCleanCommand}
          </code>
          <button
            onClick={handleCopyCleanGit}
            className="shrink-0 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-black shadow-lg flex items-center gap-1.5 transition self-end sm:self-center"
          >
            {copiedCleanGit ? <Check className="w-4 h-4 text-white" /> : <Copy className="w-4 h-4" />}
            <span>{copiedCleanGit ? 'GIT COMMAND COPIED!' : 'COPY GIT CLEAN COMMAND'}</span>
          </button>
        </div>
      </div>
